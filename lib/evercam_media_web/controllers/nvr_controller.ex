defmodule EvercamMediaWeb.NVRController do
  ...
  def reboot(conn, %{"url" => url, "port" => port, "user" => user, "password" => password}) do
    with %User{} <- conn.assigns[:current_user] do
      run_porcelain_shell(url, port, user, password, conn)
    else
      nil ->
  ...
  defp run_porcelain_shell(url, port, user, password, conn) do
    Porcelain.shell("nvr_reboot #{url} #{port} #{user} #{password}")
